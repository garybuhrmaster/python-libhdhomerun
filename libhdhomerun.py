...
_lib_name_formats = ['lib%s.so', 'lib%s.sl', '%s.so', '%s.sl']
if (sys.platform == 'darwin'): _lib_name_formats = ['lib%s.dylib', 'lib%s.so', 'lib%s.bundle', '%s.dylib','%s.so', '%s.bundle', '%s']
if (sys.platform == 'win32') or (sys.platform == 'cygwin'): _lib_name_formats = ['lib%s.dll', '%s.dll', '%slib.dll']

_libs['hdhomerun'] = None

_lib_errors = []

for _lib_name in _lib_name_formats:
    _libname = _lib_name % ('hdhomerun')
    try:
        _libs['hdhomerun'] = ctypes.CDLL(_libname)
        break
    except Exception as inst:
        inst = 'Attempting to load %s: ' % (_libname) + str(inst)
        _lib_errors.append(inst)
        pass

# Raise the error for failing to load library, displaying
# all the accumlated places that were searched....
if _libs['hdhomerun'] is None:
    _lib_load_error = ('; '.join(_lib_errors))
    raise OSError(_lib_load_error)
...
if hasattr(_libs['hdhomerun'], 'hdhomerun_device_get_lineup_location'):
    hdhomerun_device_get_lineup_location = _libs['hdhomerun'].hdhomerun_device_get_lineup_location
...
if hasattr(_libs['hdhomerun'], 'hdhomerun_device_set_lineup_location'):
    hdhomerun_device_set_lineup_location = _libs['hdhomerun'].hdhomerun_device_set_lineup_location
...
if hasattr(_libs['hdhomerun'], 'hdhomerun_discover_find_devices_custom'):
    hdhomerun_discover_find_devices_custom = _libs['hdhomerun'].hdhomerun_discover_find_devices_custom
...
if hasattr(_libs['hdhomerun'], 'hdhomerun_discover_find_devices'):
    hdhomerun_discover_find_devices = _libs['hdhomerun'].hdhomerun_discover_find_devices
...
if hasattr(_libs['hdhomerun'], 'hdhomerun_channelmap_get_channelmap_scan_group'):
    hdhomerun_channelmap_get_channelmap_scan_group = _libs['hdhomerun'].hdhomerun_channelmap_get_channelmap_scan_group
